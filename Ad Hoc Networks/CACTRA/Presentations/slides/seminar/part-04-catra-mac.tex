...
\node[below=0.25cm of s1,font=\fontsize{8}{9}\selectfont] {2 SEND flows};
\node[draw=MethodColor,circle,fill=CardBg,right=3.0cm of s1] (s2) {S2};
\node[below=0.25cm of s2,font=\fontsize{8}{9}\selectfont] {1 SEND flow};
...
\draw[-{Latex[length=2mm]},draw=MethodColor] (s2)--node[above,font=\fontsize{7}{8}\selectfont]{Flow 2}(s1);
\draw[-{Latex[length=2mm]},draw=MethodColor] (s1)--node[above,font=\fontsize{7}{8}\selectfont]{Flows 1+2}(r);
\end{tikzpicture}
\end{center}
...
\begin{frame}[shrink=3]{Real Bandwidth Ratio}
